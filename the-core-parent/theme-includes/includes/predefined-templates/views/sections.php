...
<?php if ($sections_categories): ?>
	<div class="<?php echo $prefix; ?>pred-tpl-cat">
		<label for="<?php echo esc_attr($select_id = $prefix . fw_rand_md5()) ?>"><?php
			esc_html_e('Filter by Category', 'fw') ?>:</label>
		<select id="<?php echo esc_attr($select_id) ?>" class="<?php echo $prefix; ?>pred-tpl-cat-select">
			<option value=""><?php esc_html_e('All Categories', 'fw') ?></option>
...
<?php endif; ?>
<?php if ($sections): ?>
	<div class="fw-row-spaced <?php echo $prefix; ?>pred-tpl-thumb-list">
		<?php foreach ($sections as $section_id => $section): ?>
			<div class="fw-col-sm-4 fw-col-sm-15 <?php echo $prefix; ?>pred-tpl-thumb"
