...
<div class="<?php echo esc_attr( fw_ext_builder_get_item_width( 'form-builder', $item['width'] . '/frontend_class' ) ); ?>">
	<div class="field-textarea">
		<?php if ( fw_htmlspecialchars( $item['options']['label'] ) != '' ) : ?>
			<label class="fw-col-lg-2" for="<?php echo esc_attr( $attr['id'] ) ?>"><?php echo fw_htmlspecialchars( $item['options']['label'] ); ?>
				<?php if ( $options['required'] ): ?><sup>*</sup><?php endif; ?>
			</label>
		<?php endif; ?>
		<textarea class="fw-col-lg-10" <?php echo fw_attr_to_html( $attr ) ?>><?php echo fw_htmlspecialchars( $value ); ?></textarea>
		<?php if ( $options['info'] ): ?>
